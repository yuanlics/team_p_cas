...
class RelatedCustomer():
    ...
    def get_related_customer(self, other_ws, i_ids):
        rows = self.sess.execute(self.pre_get_remote_order_lines_by_selected_items.bind((other_ws, i_ids)))
        ...
        for (ol_w_id, ol_d_id, ol_o_id), _ in at_least_two_common:
            related_customer = self.sess.execute(self.pre_get_customer_by_order.bind((ol_w_id, ol_d_id, ol_o_id))).one()
            rel_c_id = related_customer.o_c_id
            self.related_customers.add((ol_w_id, ol_d_id, rel_c_id))
        
    def exec_xact(self, w_id, d_id, c_id):
        orders = self.sess.execute(self.pre_get_orders_by_customer.bind((w_id, d_id, c_id)))
        self.related_customers = set()
        for order in orders:
            order_lines = self.sess.execute(self.pre_get_order_lines.bind((w_id, d_id, order.o_id)))
